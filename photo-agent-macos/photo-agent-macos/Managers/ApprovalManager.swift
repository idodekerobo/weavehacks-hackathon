...
class ApprovalManager: ObservableObject {
    ...
    func fetchApprovals(status: String? = nil) async {
        guard let tunnelURL = appState.tunnelURL,
            ...
            // If no tunnel URL, try localhost
            guard let baseURL = URL(string: "http://localhost:\(appState.serverPort)") else {
                error = "Invalid server URL"
                return
            }
            await performFetch(baseURL: baseURL, status: status)
            return
        }
    ...
    private func performFetch(baseURL: URL, status: String?) async {
        isLoading = true
        error = nil
        ...
        do {
            ...
            // Update pending count in app state
            let pendingCount = result.approvals.filter { $0.status == "pending" }.count
            appState.pendingApprovalsCount = pendingCount
    ...
    private func updateApprovalStatus(approvalId: String, status: String, editedData: [String: Any]? = nil) async throws {
        guard let tunnelURL = appState.tunnelURL,
            ...
            // Try localhost if no tunnel
            guard let url = URL(string: "http://localhost:\(appState.serverPort)")
                .map({ $0.appendingPathComponent("api/approvals").appendingPathComponent(approvalId) }) else {
                throw ApprovalError.invalidURL
            }
            try await performUpdate(url: url, status: status, editedData: editedData)
            return
        }
    ...
    private func performUpdate(url: URL, status: String, editedData: [String: Any]?) async throws {
